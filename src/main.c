...
int Menu_Main(void)
{
	...
	console_print_pos(0, 1, "Instalador de Indexiine Mod");
	console_print_pos(0, 2, "Traducido y modificado por Panda395 (CONTROL TOTAL)");
	console_print_pos(0, 3, "---------------------------------------------------");
    console_print_pos(0, 4, "Indexiine Installer v2 por GaryOderNichts");
	console_print_pos(0, 5, "Indexiine por Jonhyjp");
	console_print_pos(0, 6, "Indexiine Mod por Carlos Nu");
	console_print_pos(0, 7, "---------------------------------------------------");
	console_print_pos(0, 8, "Region Detectada: %s", regionStrings[region]);
	console_print_pos(0, 9, "---------------------------------------------------");

	console_print_pos(0, 11, "Boton A: Hacer backup y reemplazar el index.html");
	console_print_pos(0, 12, "Boton B: Restaurar el index.html");

	console_print_pos(0, 14, "Boton HOME: Salir");
	...
	while (1)
	{
		...
		if(vpadError == 0 && ((vpad.btns_d | vpad.btns_h) & VPAD_BUTTON_A))
		{
			...
			console_printf(1, "Indexiine Mod se ha instalado correctamente!");
			sleep(4);
			break;
		}
